...
\begin{definition}[Parameterized Curve]
    % TODO: Not sure what smoothness this has
    $\gamma$ is a \textit{parameterization} of a curve $C$ iff $\gamma\colon [a, b] \to \real^n$ is $C^1$ smooth.
\end{definition}
...
\begin{theorem}[Uniqueness of Integration up to Orientation] \label{integral_uniqueness_up_to_orientation_one_form}
    If we have paramaterizations $\gamma\colon [a, b] \to \real^n$ and $\phi\colon [a, b] \to \real^n$ for $C_1$ and $C_2$ respectively such that there exists a map $\psi\colon[a, b] \to [c, d]$ such that
    \begin{enumerate}[label = \arabic*)]
...
\begin{theorem}
    Given a curve $C$ with a parameterization $\gamma\colon [a, b] \to \real^n$ and a $1$-form $\omega$ defined on $C$ then
    \[
...
\begin{proof}
    ...
    \begin{align*}
        \int_C \omega &= \lim_{k\to\infty} \sum_{i = 1}^k \omega_{P_i}(\vec{v_i})
        = \lim_{k\to\infty} \sum_{i = 1}^k \sum_{j = 0}^n f_j(P_i) (dx_j)_P(\vec{v_i}) \\
        &= \lim_{k\to\infty} \sum_{i = 1}^k \sum_{j = 0}^n f_j(P_i)(dx_j)_P(\gamma(t_i) - \gamma(t_{i - 1})) \\
        &= \lim_{k\to\infty} \sum_{i = 1}^k \sum_{j = 0}^n f_j(P_i)(dx_j)_P(\gamma'(t_{i - 1})(t_i - t_{i -1}) + E|t_i - t_{i - 1}|) \quad \text{By Taylor's Theorem} \\
        &= \lim_{k\to\infty} \sum_{i = 1}^k \sum_{j = 0}^n f_j(P_i)(dx_j)_P(\gamma'(t_{i - 1})(t_i - t_{i -1})) \quad \text{Since $t_i - t_{i - 1} \to 0$} \\
        &= \lim_{k\to\infty} \sum_{i = 1}^k \sum_{j = 0}^n f_j(\gamma(t_{i - 1}))(dx_j)_{\gamma(t_{i - 1})}(\gamma'(t_{i - 1}))(t_i - t_{i -1}) \\
        &= \int_a^b \sum_{j = 0}^n f_j(\gamma(t))(dx_j)_{\gamma(t)}(\gamma'(t))dt
...
\begin{solution}
    \begin{align*}
        ...
        &= \int_0^1 (t^2dx_{(t, t^2)}(1,2 t) + dy_{(t, t^2)}(1, 2t))dt \\
        &= \int_0^1 (t^2 + 2t)dt
        = \frac{4}{3}
    \end{align*}
\end{solution}
